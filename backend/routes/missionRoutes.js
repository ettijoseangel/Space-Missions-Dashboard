...
router
  ...
  .post(
    protect,
    adminOnly,
    validateData(missionValidationSchema),
    createMission,
  ); // Crea una nueva mision - ACCESO RESTRINGIDO
...
router
  ...
  // ¿Quién eres? -> ¿Eres Admin? -> ¿Datos válidos? -> Actualiza DB
  .put(
    protect,
    adminOnly,
    validateData(missionValidationSchema),
  ...
  // ¿Quién eres? -> ¿Eres Admin? -> Borra de DB
  .delete(
    protect,
    adminOnly,
    deleteMission
